...
async function commentedOnPlaylistAll(comment_owner, playlist_id, comment_id) {
  ...
  const all_comments = await db.select('*').
    from('review').
    where('playlist_id', playlist.id);

  return all_comments.map(comment => {
      if (comment.user_id !== comment_owner && comment.user_id !== playlist.user_id) {
        return {
          ...
          title: `${comment_sender.alias || comment_sender.first_name || 'Some mysterious user'} just added a comment on the playlist ${playlist.title}.`,
          status: 'unread',
